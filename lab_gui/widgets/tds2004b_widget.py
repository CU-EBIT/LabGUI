...
# Conversion from reported ADC value to screen coordinates
to_screen_divs = 1 / (0.1*(1<<16))

class TDS2004B(DeviceReader):
    '''A Tektronix TDS 2004B Four Channel Digital Storage Oscilloscope
    ...
    '''
    def __init__(self, parent, addr, channels=['ch1', 'ch2', 'ch3', 'ch4']):
        """_summary_

        Args:
            parent (FigureModule): the module we are made from
            addr (str): pyvisa address of the scope
            channels (list, optional): channels to display. Defaults to ['ch1', 'ch2', 'ch3', 'ch4'].
        """
        ...
        super().__init__(parent, data_key=None, name=f"TDS2004B", axis_title=f"Signal (V)")
        ...
        def one_two_five(old, new, values=[1.0, 2.5, 5.0]):
        ...
        def addChannelCtrl(channel):
            ...
            def update_channel(*_):
                ...
                def do_update():
                    self.device.write(f'{channel}:scale {value}')
                    self.scale[channel] = value * self.probe[channel]
                self.queue_cmd(do_update)
            ...
            outer.addWidget(QLabel(f"{channel} Control"))
    ...
    def open_device(self):
        ...
        try:
            ...
            # Setup block
            self.device.write('horizontal:recordlength 100000')
            self.device.write('data:resolution full')
            self.device.write('data:width 2')
            self.device.write('data:enc RIB')

            self.x = float(self.device.query("horizontal:main:scale?"))

            for channel in self.channels:
                self.device.write(f'data:source {channel}')
                # The below lines take 300-500ms to run, so only do it rarely
                scale = float(self.device.query(f'{channel}:scale?'))
                pos = float(self.device.query(f'{channel}:position?'))
                probe = float(self.device.query(f'{channel}:probe?')) 
                self.scale[channel] = scale
                self.probe[channel] = probe
                self.pos[channel] = pos

                self.scale_box[channel].box.setText(f"{scale / probe:.1e}")
    ...
    def acquire(self, channel):
        ...
        # This translates them down to 0
        vars = numpy.array(vars) * to_screen_divs - self.pos[channel]
        # Now scale them by scale
        vars = numpy.array(vars) * self.scale[channel]
